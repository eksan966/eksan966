...
function Library:CreateWindow(...)
    ...
    if typeof(Config.Position) ~= 'UDim2' then 
        if IsMobile then
            Config.Position = UDim2.fromOffset(0, 0)
        else
            Config.Position = UDim2.fromOffset(180, 50)
        end
    end
    if typeof(Config.Size) ~= 'UDim2' then
        if IsMobile then
            -- Use full screen on mobile devices
            Config.Size = UDim2.fromScale(1, 1)
            Config.Size = UDim2.new(Config.Size.X.Scale, Config.Size.X.Offset - 10, Config.Size.Y.Scale, Config.Size.Y.Offset - 10)
        else
            Config.Size = UDim2.fromOffset(600, 630)
        end
    end
...
function Groupbox:Resize()
    ...
    -- Mobile double-tap to open menu
    local LastTapTime = 0
    local TapPosition = nil
    ...
    Library:GiveSignal(InputService.InputBegan:Connect(function(Input, Processed)
        if IsMobile and Input.UserInputType == Enum.UserInputType.Touch then
            ...
            if TapPosition and (CurrentTime - LastTapTime < 0.5) then
                local Distance = math.sqrt((CurrentPosition.X - TapPosition.X)^2 + (CurrentPosition.Y - TapPosition.Y)^2)
